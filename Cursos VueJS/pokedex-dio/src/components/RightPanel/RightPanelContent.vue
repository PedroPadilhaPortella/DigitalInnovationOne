<template>
    <div class="search">
        <label for="search" class="search__label text--white bg--black">
            ...
            <input type="text" id="search" class="search__input" v-model.trim="name" @keyup.enter="searchPokemon">
        </label>
        <div class="buttons">
            <button class="btn btn--clear" @click.prevent="clear">Clear</button>
            <button class="btn btn--search bg--gray text--yellow" @click.prevent="searchPokemon">Search</button>
        </div>
    </div>
</template>
